...
- `networking.nix`
  - If using static assignment, the `interface` parameter needs to be set to whatever the interface is in the VM (typically it's just `ens18`)
- `nas-host-configuration.nix`
  - `fileSystems."/mnt/homelab-data".device` needs to be set to `/dev/disk/by-uuid/<DISK_UUID>`. Disk UUID can be found with `lsblk -f`
...
- Copy this directory into the host
